...
\newmdtheoremenv[everyline=true,linewidth=1pt,backgroundcolor=shallowGreen,linecolor=deepGreen,innertopmargin=1pt,leftmargin=1pt,innerleftmargin=20pt,innerrightmargin=20pt]{theorem}{Théorème}[subsection]
\theoremstyle{mytheoremstyle}
\newmdtheoremenv[everyline=true,linewidth=1pt,backgroundcolor=shallowRed,linecolor=deepRed,innertopmargin=1pt,leftmargin=1pt,innerleftmargin=20pt,innerrightmargin=20pt,]{prop}{Proposition}[subsection]
\theoremstyle{mytheoremstyle}
\newmdtheoremenv[everyline=true,linewidth=1pt,backgroundcolor=lilac,linecolor=deepPurple,innertopmargin=1pt,leftmargin=1pt,innerleftmargin=20pt,innerrightmargin=20pt,]{definition}{Définition}[subsection]
\theoremstyle{mytheoremstyle}
\newmdtheoremenv[everyline=true,linewidth=1pt,backgroundcolor=shallowBlue,linecolor=deepBlue,innertopmargin=1pt,leftmargin=1pt,innerleftmargin=20pt,innerrightmargin=20pt,]{ef}{EF}[subsection]
\theoremstyle{mytheoremstyle}
\newmdtheoremenv[everyline=true,linewidth=1pt,backgroundcolor=shallowBlue, linecolor=deepBlue,innertopmargin=1pt,leftmargin=1pt, innerleftmargin=10pt, innerrightmargin=10pt,]{st}{S}[subsection]
\theoremstyle{mytheoremstyle}
\newmdtheoremenv[everyline=true,linewidth=1pt,backgroundcolor=mygray,linecolor=black,leftmargin=1pt,innertopmargin=1pt,innerleftmargin=20pt,innerrightmargin=20pt,]{ex}{}[subsection]

\theoremstyle{mytheoremstyle}
\newmdtheoremenv[everyline=true,linewidth=1pt,backgroundcolor=shallowBlue,linecolor=deepBlue,innertopmargin=1pt,leftmargin=1pt,innerleftmargin=20pt,innerrightmargin=20pt,]{rmq}{Remarque}[subsection]

\theoremstyle{mytheoremstyle}
\newmdtheoremenv[everyline=true,linewidth=1pt,backgroundcolor=mygray,linecolor=black,innertopmargin=1pt,leftmargin=1pt,innerleftmargin=20pt,innerrightmargin=20pt,]{exe}{Exercice}[subsection]


\theoremstyle{myproblemstyle}
\newmdtheoremenv[everyline=true,linecolor=black,leftmargin=1pt,innerleftmargin=10pt,innerrightmargin=10pt,]{problem}{Problem}[subsection]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{definition}[Produit Scalaire Canonique de $\bR^n$]
  Soit $(x,y) \in \left(\bR^n\right)^2$, soit $X,Y$ tels que 
  \begin{align*}
    &X = \text{Mat}_{\mfc B} x \\ 
    &Y = \text{Mat}_{\mfc B} y
  \end{align*}
  alors on défini le produit scalaire canonique de $\bR^n$ comme 
  \begin{align*}
    x \cdot y &= ~^tXY \\ 
    &= \sum_{i=1}^n [X]_{i,i} [Y]_{i,i} 
  \end{align*}
\end{definition}
\begin{rmq}
  Il est parfois noté $<x,y>$ ou encore $(x|y)$
\end{rmq}
\begin{rmq}
  Le produit scalaire est une forme bilinéaire sur $\bR^n$, en particulier
  \begin{align*}
    &\forall (x,y) \in \left(\bR^n\right)^2, \lambda \in \bR, (\lambda x) \cdot y = x \cdot (\lambda y) = \lambda (x \cdot y) \\ 
    &\forall (x,y,z) \in \left(\bR^n\right)^3, (x+z) \cdot y = (x \cdot y) + (z \cdot y) 
    &\forall (x,y) \in \left(\bR^n\right)^2, x \cdot y = y \cdot x 
  \end{align*}
  \begin{proof}
    Par définition d'un produit scalaire (voir \hyperref[subsubsec:h1]{ici}), un produit scalaire est une forme bilineaire, symétrique, définie, positive
  \end{proof}
\end{rmq}
\begin{ex}
  Soit $x,y$ tels que 
  ...
  alors on a 
  \begin{equation*}
    x \cdot y = 4 \cdot -1 + 1 \cdot 2 + 5 \cdot 1 = 3
  \end{equation*}
\end{ex}
\section{Norme}
\begin{definition}[Norme euclidienne de $\bR^n$]
  Soit $x \in \bR^n$, on appelle norme euclidienne de $x$ le réel 
  \begin{equation*}
    \norm{x} = \sqrt{x \cdot x}
  \end{equation*}
  Qui existe par positivité d'un produit scalaire.
\end{definition}
\begin{rmq}
  Une norme est une forme, définie, positive, homogène voir \hyperref[subsubsec:h2]{ici}, en particulier
  \begin{equation*}
    \norm{x} = 0 \Leftrightarrow x = 0_E
  \end{equation*}
\end{rmq}
\begin{rmq}
  On dit d'un vecteur dont la norme est $1$ qu'il est \textit{unitaire} ou \textit{normé}
\end{rmq}
\begin{ex}
  Soit $x$ définie par 
  \begin{equation*}
  ...
  \begin{align*}
    \norm{x} &= \sqrt{x \cdot x} \\ 
    &= \sqrt{3 \cdot 3 + (-1)^2 + 2^2} \\ 
    &= \sqrt{14}
  \end{align*}
\end{ex}
\section{Cauchy-Swartz}
\begin{theorem}[Inégalité de Cauchy-Swartz]
...
\end{prop}
\subsection{Angle entre vecteurs}
\begin{definition}
  D'après Cauchy-Swartz on a 
  \begin{equation*}
    \frac{\abs{x \cdot y}}{\norm{x}\norm{y}} < 1
  \end{equation*}
  donc 
  ...
  d'où
  \begin{equation*}
    \exists! \theta \in [0,\pi], \cos \theta = \frac{x \cdot y}{\norm{x}\norm{y}}
  \end{equation*}
  on appelle cet angle, l'angle entre les vecteurs $x$ et $y$
\end{definition}
\subsubsection{Définitions}
\begin{definition}
  Deux vecteurs $x$ et $y$ sont orthogonaux si et seulement si leur angle et $\frac{\pi}{2}$ donc 
  \begin{equation*}
    x \cdot y = 0
  \end{equation*}
\end{definition}
\begin{prop}
  Si $x \in E$ est orthogonal à une famille $(v_1,\cdots,v_p)$ de vecteurs alors $x$ est orthogonal à toute combinaison linéaire de cette famille 
  \begin{proof}
    directe par définition
  \end{proof}
\end{prop}
\begin{definition}
  Deux vecteurs $x$ et $y$ sont orthonormés si 
  \begin{align*}
    x \cdot y = 0 \\ 
    \norm{x} = 1 \\ 
    \norm{y} = 1
...
\end{definition}
\begin{definition}
  Une famille $(v_1,\cdots,v_p)$ de vecteurs est orthogonale si et les vecteurs sont deux à deux orthogonaux
\end{definition}
\begin{definition}
  Une famille $(v_1,\cdots,v_p)$ est orthonormée si elle est orthogonale et que chaque vecteur est normé.
\end{definition}
\begin{prop}
  Une famille orthonormée est libre.
  \begin{proof}
    Soit $(v_1,\cdots,v_p)$ une famille orthonormée, alors soit $(\alpha_1,\cdots,\alpha_p)$ telle que 
    $\sum_{i=1}^n \alpha_i v_i = 0_E$
    \begin{align*}
      v_1 \cdot (\sum_{i=1}^p \alpha_i v_i) &= \alpha_1 v_1 \cdot v_1 + \sum_{i=2}^p \alpha_i v_1 \cdot v_i \\ 
      &= \alpha_1 \norm{v_1}^2 + 0 = 0 && \text{car la famille est orthogonale} \\ 
      &= \alpha_1 = 0 && \text{car la famille est normée}
    \end{align*}
    de la même manière on prouve que $(\alpha_1,\cdots,\alpha_p) = (0,\cdots,0)$ 
  \end{proof}
\end{prop}
\begin{rmq}
  Si la famille $(v_1,\cdots,v_p)$ est orthogonale alors la famille $(\frac{v_1}{\norm{v_1}},\cdots,\frac{v_p}{\norm{v_p}})$ est orthonormée
\end{rmq}
\subsection{Orthogonalisation}
\begin{theorem}[Algorithme de Gram-Schmidt]
  Soit $(u_1,\cdots,u_p)$ libre alors il existe $(v_1,\cdots,v_p)$ orthogonale telle que 
  \begin{align*}
    \text{Vect}(u_1) &= \text{Vect}(v_1) \\ 
    \vdots &= \vdots \\ 
    \text{Vext}(u_1,\cdots,u_p) &= \text{Vect}(v_1,\cdots,v_p)
  \end{align*}
  \begin{proof}
    À l'état initial on prend $v_1 = u_1$.\newline

    On pose $v_2 = u_2 + \alpha v_1$ on a tout de suite que 
    \begin{equation*}
      \text{Vect}(u_1,u_2) = \text{Vect}(v_1,v_2)
    \end{equation*}
    ensuite 
    \begin{align*}
      v_2 \cdot v_1 &= (u_2 + \alpha v_1) \cdot v_1 \\ 
      &= u_2 \cdot v_1 + \alpha \norm{v_1}^2
    \end{align*}
    On prend donc $\alpha = \frac{-u_2\cdot v_1}{\norm{v_1}^2}$ et on a bien l'orthogonalité\newline

    On itère jusqu'à $p$, on a comme formule générale à la fin que 
    \begin{equation*}
      \alpha_i = - \frac{u_p\cdot v_i}{\norm{v_i}^2}
    \end{equation*}
  \end{proof}
\end{theorem}
\section{Théorème de Pythagore}
\begin{theorem}{Théorème de Pythagore - Version généralisée}
  Soit $(u_1,\cdots,u_p)$ orthogonale alors 
  \begin{equation*}
    \norm{u_1 + \cdots + u_p}^2 = \sum_{i=1}^p \norm{u_i}^2
  \end{equation*}
  \begin{proof}
    Cas où n=2, 
    \begin{align*}
     \norm{u_1 + u_2}^2 &= (u_1 + u_2)\cdot (u_1 + u_2) \\ 
      &= \norm{u_1}^2 + 2 (u_1 \cdot u_2) + \norm{u_2}^2 && \text{or la famille est orthogonale} \\ 
      &= \norm{u_1}^2 + \norm{u_2}^2
    \end{align*}
    Cas où n=3,
    \begin{align*}
      \norm{u_1 + u_2 + u_3}^2 &= \norm{(u_1+u_2) + u_3}^2 \\ 
      &= \norm{u_1+u_2}^2 + \norm{u_3}^2 && \text{cas n=2} \\ 
      &= \norm{u_1}^2 + \norm{u_2}^2 + \norm{u_3}^2 && \text{cas n=2}
    \end{align*}
    Cas où n=p, on se ramène par récurrence à l'ordre 2.
  \end{proof}
\end{theorem}
\section{Projection orthogonale sur une droite}
\begin{definition}
  Soit $\Delta$ une droite de coefficient directeur $u$ normé, soit $V \in E$ on appelle projection orthogonale sur $\Delta$ de $V$ le vecteur 
  \begin{equation*}
   p = (V \cdot u) u
  \end{equation*}
  \begin{proof}
    On cherche p la projection orthogonale de $V$ sur $\Delta$, on a $V = \alpha u + V_1$
    \begin{align*}
      V \cdot u &= \alpha u \cdot u + v_1 \cdot u \\ 
      &= \alpha \norm{u}^2 + V_1 \cdot u \\ 
      &= \alpha && \text{car V_1 est orthogonale à u}
    \end{align*}
    On a donc 
    \begin{equation*}
       p = (V \cdot u) u
...
\end{definition}
\begin{ex}
  Soit $\Delta$ de coefficient directeur $u = \begin{pmatrix}
    2 \\ 
    1
  \end{pmatrix}$ et $v = \begin{pmatrix}
    4 \\
  ...
  On a alors 
  \begin{align*}
    v \cdot u_1 &= 4 \cdot \frac{2}{\sqrt{5}} + 7 \cdot \frac{1}{\sqrt{5}}
    &= \frac{15}{\sqrt{5}}
  \end{align*}
  Donc enfin on a 
  \begin{align*}
    p &= \frac{15}{\sqrt{5}} u_1 \\ 
    &= \begin{pmatrix}
      6 \\ 
      3
    \end{pmatrix}
  \end{align*}
\end{ex}
\section{Changement de bases orthogonales}
\begin{definition}
  On définie les matrices orthogonales comme étant les matrices telles que 
  \begin{equation*}
    P^{-1} = ~^tP
  \end{equation*}
  cela découle des matrices de changement de bases orthogonales.
\end{definition}
